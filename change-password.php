...
$udate=date('Y-m-d h:i:s', time());;
	$sql="SELECT password FROM user_registration where password=?";
	...
	if($row_cnt>0)
	{
		$con="update user_registration set password=?  where user_id=?";
...
<?php $result ="SELECT passUpdateDate FROM user_registration WHERE user_id=?";
